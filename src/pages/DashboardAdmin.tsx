  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className={`min-w-0 transition-all duration-300`} style={{ marginLeft: sidebarWidth }}>
        <AppTopbar title="Panel Admin" sidebarOpen={sidebarOpen} onMenuClick={() => setSidebarOpen(true)} />
        ...
        <main className="p-8 pb-[60px] max-md:p-5 relative">
          ...
          <section className="rounded-2xl p-10 max-md:p-6 mb-[26px] relative overflow-hidden border border-white/10">
            ...
            <div className="relative z-10">
              ...
              <p className="text-sm text-text-muted mb-4">Gestioná torneos, equipos, calendarios y configuración general.</p>
          ...
          <section className="grid grid-cols-4 max-lg:grid-cols-2 gap-[18px] mb-[26px]">
            {[
              ...
              { icon:'👥', num:'32', label:'Equipos registrados', accent:'gold' },
              { icon:'📅', num:'48', label:'Partidos programados', accent:'purple' },
              { icon:'👤', num:'256', label:'Usuarios activos', accent:'gold' },
            ].map((s, i) => (
          ...
          <section className="flex items-center justify-between gap-5 p-[26px_30px] rounded-[18px] bg-gradient-to-r from-purple-deep2 to-purple-black border border-white/10 max-md:flex-col max-md:items-start">
            <div>
              <h3 className="font-[family-name:var(--font-display)] uppercase text-xl leading-tight mb-1">Configuración del torneo</h3>
              <p className="text-[13px] text-text-muted">Creá torneos, gestioná equipos y administrá el calendario.</p>
            </div>
            <InteractiveHoverButton onClick={() => navigate('/torneos')}>
              Administrar torneos
