    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-[440px] relative z-10">
        <Link to="/" className="flex items-center gap-2.5 mb-8 justify-center">
          ...
          <span className="font-[family-name:var(--font-display)] font-bold text-lg">TECH<span className="text-gold-ink">CUP</span></span>
        </Link>
          ...
          <AnimatePresence mode="wait">
            {step === 'email' && (
              <motion.div key="email" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <div className="w-14 h-14 rounded-full bg-purple-mid/20 border border-purple-mid/30 flex items-center justify-center mx-auto mb-6">
                  <Lock size={24} className="text-gold-ink" />
                </div>
                <h1 className="font-[family-name:var(--font-display)] text-2xl uppercase tracking-[.5px] text-center mb-2">¿Olvidaste tu <span className="text-gold-ink">contraseña?</span></h1>
                ...
                <div className="text-center mt-6">
                  <Link to="/login" className="text-sm text-text-muted hover:text-gold-ink inline-flex items-center gap-1.5"><ArrowLeft size={14} /> Volver al inicio de sesión</Link>
                </div>
              </motion.div>
            )}
            ...
            {step === 'otp' && (
              <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="text-center">
                <div className="w-14 h-14 rounded-full bg-purple-mid/20 border border-purple-mid/30 flex items-center justify-center mx-auto mb-6">
                  <KeyRound size={24} className="text-gold-ink" />
                </div>
                <h1 className="font-[family-name:var(--font-display)] text-2xl uppercase tracking-[.5px] mb-2">Verificá tu <span className="text-gold-ink">correo</span></h1>
                ...
                <p className="text-xs text-text-muted mb-6">¿No recibiste el código? <button className="text-gold-ink font-semibold hover:underline">Reenviar</button></p>
            ...
            {step === 'newpass' && (
              <motion.div key="newpass" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <h1 className="font-[family-name:var(--font-display)] text-2xl uppercase tracking-[.5px] text-center mb-2">Nueva <span className="text-gold-ink">contraseña</span></h1>
