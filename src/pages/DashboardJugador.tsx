    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="min-w-0 transition-all duration-300" style={{ marginLeft: sidebarWidth }}>
        ...
        <main className="p-8 pb-[60px] max-md:p-5 relative" style={{ backgroundImage: 'linear-gradient(rgba(30,10,60,0.65), rgba(30,10,60,0.65)), url(/images/fondo3.png)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
            ...
            <SpotlightCard accent="purple" className="p-[22px_24px] bg-[#E8DFF5]/70 dark:bg-black/40 backdrop-blur-sm border border-[#D4C8E8]/40 dark:border-purple-mid/30 rounded-2xl shadow-lg shadow-purple-mid/10">
              ...
              {misPartidos.map((m, i) => (
                <Link key={i} to={`/partido/${i + 1}`} className="flex items-center gap-3.5 py-3 border-b border-gold/10 last:border-b-0 hover:bg-black/5 dark:hover:bg-white/[0.05] transition-all rounded-lg -mx-2 px-2 group">
                  ...
                  <div className="flex-1 text-[13.5px] font-semibold text-[#3D1A6B] dark:text-white">
          ...
          {isCaptain && (
            <>
              ...
              <section className="relative rounded-2xl overflow-hidden mb-5 border border-white/10">
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-[rgba(10,6,20,0.8)] via-[rgba(10,6,20,0.6)] to-transparent" />
                <div className="absolute inset-0 bg-gradient-to-t from-[rgba(10,6,20,0.9)] via-transparent to-[rgba(10,6,20,0.4)]" />
              ...
              {/* Tabs capitán */}
              <div className="flex items-center gap-1 mb-6 bg-[#E8DFF5]/70 dark:bg-white/5 border border-[#D4C8E8]/40 dark:border-white/10 rounded-xl p-1 overflow-x-auto">
                {([
                    ...
                    <SpotlightCard accent="purple" className="p-[22px_24px] bg-surface border-border rounded-2xl">
                      ...
                      {partidos.slice(0, calExpandido ? partidos.length : 2).map((m, i) => (
                        <button key={i} onClick={() => navigate(`/partido/${i + 1}`)}
                          className="w-full flex items-center gap-3.5 py-3 border-b border-border last:border-b-0 hover:bg-black/5 dark:hover:bg-white/[0.03] transition-colors rounded-lg -mx-2 px-2 text-left">
                          <div className="w-[52px] text-center flex-shrink-0">
                            <b className="block font-[family-name:var(--font-display)] text-lg text-[#3D1A6B] dark:text-white">{m.dia}</b>
                            <span className="text-[10px] text-text-muted uppercase">{m.mes}</span>
                          </div>
                          <div className="flex-1 text-[13.5px] font-semibold text-[#3D1A6B] dark:text-white">
                            {m.eq1} <span className="text-text-faint">vs</span> {m.eq2}
                  ...
                  <SpotlightCard accent="purple" className="p-[22px_24px] bg-surface border-border rounded-2xl">
                    ...
                    <div className="grid grid-cols-2 max-md:grid-cols-1 gap-2">
                      {MI_EQUIPO.jugadores.map(j => (
                        <div key={j.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-[#E8DFF5]/70 dark:bg-black/30 border border-[#D4C8E8]/40 dark:border-white/5">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[11.5px] text-text-muted leading-relaxed">
                          1. El capitán genera un código QR desde la sección <strong className="text-[#3D1A6B] dark:text-white">QR Dotación</strong>.<br />
                          ...
                          <div className="flex items-center gap-3">
                            {!qrEntregado ? (
                              <>
                                ...
                                <Button variant="outline" size="sm" className="rounded-full border-[#D4C8E8]/40 dark:border-white/20 text-[#3D1A6B] dark:text-white hover:bg-black/5 dark:hover:bg-white/10 text-xs h-9 px-4">
                              ...
                              <>
                                ...
                                <Button onClick={() => { setQrGenerado(false); setQrEntregado(false) }} variant="outline" size="sm" className="rounded-full border-[#D4C8E8]/40 dark:border-white/20 text-[#3D1A6B] dark:text-white hover:bg-black/5 dark:hover:bg-white/10 text-xs h-9 px-4">
                  ...
                  <SpotlightCard accent="purple" className="p-[22px_24px] bg-surface border-border rounded-2xl">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-[#E8DFF5]/70 dark:bg-black/30 border border-[#D4C8E8]/40 dark:border-white/5">
          ...
          {/* CTA */}
          <section className="flex items-center justify-between gap-5 p-[26px_30px] rounded-[18px] bg-[#E8DFF5]/70 dark:bg-black/50 backdrop-blur-sm border border-[#D4C8E8]/40 dark:border-white/10 mt-6 max-md:flex-col max-md:items-start">
            <div>
        ...
        {editingProfile && (
          ...
          >
            <motion.div initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 20 }}
              ...
              className="bg-[#E8DFF5]/70 dark:bg-black/60 backdrop-blur-lg border border-[#D4C8E8]/40 dark:border-purple-mid/40 rounded-3xl w-full max-w-lg shadow-2xl shadow-gold/10 p-6"
              onClick={e => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-[family-name:var(--font-display)] text-xl uppercase tracking-[.5px] text-[#3D1A6B] dark:text-white">Editar <span className="text-gold">perfil</span></h2>
                <button onClick={() => setEditingProfile(false)} className="w-8 h-8 rounded-full bg-black/5 dark:bg-white/5 border border-[#D4C8E8]/40 dark:border-white/10 flex items-center justify-center hover:bg-red-500/20 hover:border-red-500/30 transition-colors"><X size={16} className="text-gray-light" /></button>
              </div>
              <div className="flex items-center gap-4 mb-6">
                <div className="relative">
                  ...
                  <button className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-purple-mid border-2 border-[#E8DFF5] dark:border-black flex items-center justify-center"><Camera size={10} className="text-white" /></button>
                </div>
                <div className="text-sm">
                  <p className="font-semibold text-[#3D1A6B] dark:text-white">{user?.name || 'Usuario'}</p>
              ...
              <div className="space-y-4 mb-6">
                <div>
                  ...
                  <Input value={editName} onChange={e => setEditName(e.target.value)} className="bg-[#E8DFF5]/70 dark:bg-black/40 border-[#D4C8E8]/40 dark:border-purple-mid/30 text-[#3D1A6B] dark:text-white rounded-xl h-11 focus-visible:border-gold" />
                </div>
                <div className="relative">
                  <label className="block text-xs text-gold/60 font-semibold uppercase tracking-[.4px] mb-1.5">Correo</label>
                  <Input defaultValue={user?.email || 'correo@escuelaing.edu.co'} className="bg-[#E8DFF5]/70 dark:bg-black/40 border-[#D4C8E8]/40 dark:border-purple-mid/30 text-[#4B2D7A] dark:text-white/60 rounded-xl h-11 pr-10" disabled />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select value={editProgram} onChange={e => setEditProgram(e.target.value)} className="w-full bg-[#E8DFF5]/70 dark:bg-black/40 border border-[#D4C8E8]/40 dark:border-purple-mid/30 text-[#3D1A6B] dark:text-white rounded-xl h-11 px-3 text-sm outline-none focus:border-gold">
                  ...
                  <div>
                    ...
                    <select value={editSemester} onChange={e => setEditSemester(e.target.value)} className="w-full bg-[#E8DFF5]/70 dark:bg-black/40 border border-[#D4C8E8]/40 dark:border-purple-mid/30 text-[#3D1A6B] dark:text-white rounded-xl h-11 px-3 text-sm outline-none focus:border-gold">
              ...
              <div className="flex gap-3 justify-end">
                <Button variant="outline" onClick={() => setEditingProfile(false)} className="rounded-full bg-black/5 dark:bg-white/5 backdrop-blur-md border border-[#D4C8E8]/40 dark:border-purple-mid/40 text-gold hover:bg-gold/15 hover:border-gold/60 hover:text-white h-11 px-6 text-sm">Cancelar</Button>
      ...
      {torneoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setTorneoModal(null)}>
          ...
          <div className="relative max-w-lg w-full bg-white dark:bg-[#1D0E33] rounded-2xl overflow-hidden border border-[#D4C8E8]/40 dark:border-white/10 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {browseTeamsOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] flex items-center justify-center p-4" onClick={() => setBrowseTeamsOpen(false)}>
            ...
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} transition={{ duration: 0.25 }}
              className="relative w-full max-w-2xl bg-white dark:bg-[#1A0D2E] border border-[#D4C8E8]/40 dark:border-white/10 rounded-3xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              <div className="px-6 pt-4 pb-2">
                <div className="flex items-center gap-2 bg-black/5 dark:bg-white/5 border border-[#D4C8E8]/40 dark:border-white/10 rounded-xl px-3.5 py-2.5 focus-within:border-gold/40 transition-all">
                  <Search size={14} className="text-text-faint shrink-0" />
                  <input value={teamSearch} onChange={e => setTeamSearch(e.target.value)} placeholder="Buscar equipo por nombre..." className="bg-transparent border-none outline-none text-[#3D1A6B] dark:text-white placeholder:text-text-faint text-sm w-full" />
                </div>
              </div>
              <div className="max-h-[45vh] overflow-y-auto p-6 pt-2 space-y-3">
                {equiposDisponibles.filter(eq => eq.nom.toLowerCase().includes(teamSearch.toLowerCase())).map((eq, i) => (
                  <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-[#E8DFF5]/70 dark:bg-black/30 backdrop-blur-sm border border-[#D4C8E8]/40 dark:border-purple-mid/20 hover:border-purple-mid/50 transition-all group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <h4 className="text-sm font-bold text-[#3D1A6B] dark:text-white">{eq.nom}</h4>
