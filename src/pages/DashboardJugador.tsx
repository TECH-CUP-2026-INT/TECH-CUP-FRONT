        ...
        <main className="p-8 pb-[60px] max-md:p-5 relative" style={{ backgroundImage: 'linear-gradient(rgba(30,10,60,0.65), rgba(30,10,60,0.65)), url(/images/fondo3.png)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
          ...
          <section className="rounded-2xl mb-[26px] relative overflow-hidden border border-purple-mid/30" style={{ minHeight: '280px' }}>
            ...
            <div className="relative z-10 flex items-center gap-6 flex-wrap px-10">
              ...
              <Button onClick={() => { setEditingProfile(true); setEditName(user?.name || '') }} className="rounded-full bg-gold/10 border border-purple-mid/40 text-gold-ink hover:bg-gold/20 text-sm">
                Editar perfil
              </Button>
            </div>
          </section>
          ...
          <section className="grid grid-cols-3 max-lg:grid-cols-1 gap-5 mb-[26px]">
              ...
              {isCaptain ? (
                ...
                <div className="text-center py-4">
                  ...
                  <div className="flex gap-2 justify-center">
                    ...
                    <Button onClick={handleCrearEquipo} variant="outline" className="rounded-full bg-white/5 backdrop-blur-md border border-purple-mid/40 text-gold-ink hover:bg-gold/15 hover:border-gold/60 hover:text-white text-xs px-4">Crear equipo</Button>
                  </div>
                </div>
              )}
            </SpotlightCard>
            ...
            {/* Invitaciones */}
            <SpotlightCard accent="purple" className="p-5 bg-black/40 backdrop-blur-sm border border-purple-mid/30 rounded-2xl shadow-lg shadow-purple-mid/10">
              <h3 className="text-[14.5px] font-semibold tracking-[.3px] mb-3 flex items-center gap-2 text-white"><span className="text-gold-ink">📨</span> <span className="text-gold-ink">Invitaciones</span></h3>
              <div className="text-center py-4">
                <p className="text-4xl mb-2 opacity-60">🔔</p>
              ...
              <div className="space-y-3">
                {torneosDisponibles.map((t) => (
                  <button key={t.id} onClick={() => { setTorneoModal(t); setModalTab('info') }}
                    ...
                    <div>
                      <p className="text-sm font-semibold text-white group-hover:text-gold-ink transition-colors">{t.nombre}</p>
                ...
                {([
                  ...
                  <button key={t.id} onClick={() => setCapitanTab(t.id)}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[12.5px] font-semibold transition-all whitespace-nowrap ${
                      capitanTab === t.id ? 'bg-gold/20 backdrop-blur-md border border-gold/40 text-gold-ink' : 'text-white/50 backdrop-blur-sm hover:text-white hover:bg-white/10'
                    }`}>
          ...
          >
            ...
            >
              ...
              <div className="flex gap-3 justify-end">
                <Button variant="outline" onClick={() => setEditingProfile(false)} className="rounded-full bg-white/5 backdrop-blur-md border border-purple-mid/40 text-gold-ink hover:bg-gold/15 hover:border-gold/60 hover:text-white h-11 px-6 text-sm">Cancelar</Button>
                <Button onClick={handleSaveProfile} className="rounded-full bg-gold/15 backdrop-blur-md border border-gold/40 text-gold-ink hover:bg-gold/25 hover:text-white shadow-lg shadow-gold/10 font-bold h-11 px-6 text-sm">Guardar cambios</Button>
              </div>
            </motion.div>
          </motion.div>
        )}
            ...
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} transition={{ duration: 0.25 }}
              ...
              <div className="px-8 pt-10 pb-6 text-center border-b border-white/5">
                ...
                <h2 className="font-[family-name:var(--font-display-alt)] font-bold text-[clamp(28px,3.5vw,42px)] leading-[.92] tracking-[.5px] uppercase italic mb-3">
                  <span className="text-white">Explorá los</span> <span className="text-gold-ink">equipos</span>
                </h2>
                ...
                {equiposDisponibles.filter(eq => eq.nom.toLowerCase().includes(teamSearch.toLowerCase())).map((eq, i) => (
                  <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-black/30 backdrop-blur-sm border border-purple-mid/20 hover:border-purple-mid/50 transition-all group">
                    ...
                    <button disabled={eq.vac === 0}
                      ...
                      className={`shrink-0 px-4 py-2 rounded-full text-xs font-bold transition-all ${eq.vac > 0 ? 'bg-gold/20 text-gold-ink hover:bg-gold/30 cursor-pointer' : 'bg-white/5 text-white/30 cursor-not-allowed'}`}>
                      Unirme
                    </button>
                  </div>
                ))}
